...
# Initialize the T5 paraphrasing model
...
# Function to get synsets for a word using WordNet
...
def paraphrase_query(query):
    paraphrased_queries = []
    #paraphrased = paraphraser(f"paraphrase: {query}", max_length=50, num_return_sequences=3)
...
# Combine both augmentation techniques
...
def match_intent(user_query):
    ...
    # Process each augmented query
    for augmented_query in augmented_queries:
        user_doc = nlp(augmented_query.lower())  # Convert user query to lowercase and process it
        for intent, phrases in intent_phrases.items():
            for phrase in phrases:
                phrase_doc = nlp(phrase.lower())  # Convert phrase to lowercase and process it
                similarity = user_doc.similarity(phrase_doc)

                # Log similarity for debugging
                print(f"Comparing '{augmented_query}' with '{phrase}', similarity: {similarity}")

                # Check for synsets overlap on certain keywords
                if check_synsets_overlap(augmented_query, phrase):
                    print(f"Synsets overlap found between '{augmented_query}' and '{phrase}'")
                    return intent  # Return the matched intent if overlap found
    ...
    if highest_similarity > 0.6:  # Adjusted to 0.6 for more flexible matching
        return best_match
    else:
        return "fallback"

# Function to check for synset overlap between user query and intent phrases
def check_synsets_overlap(user_query, phrase):
    # Focus on important words like nouns and verbs
    user_doc = nlp(user_query)
    phrase_doc = nlp(phrase)

    user_keywords = [token.text for token in user_doc if token.pos_ in ['NOUN', 'VERB']]
    phrase_keywords = [token.text for token in phrase_doc if token.pos_ in ['NOUN', 'VERB']]

    for word1 in user_keywords:
        for word2 in phrase_keywords:
